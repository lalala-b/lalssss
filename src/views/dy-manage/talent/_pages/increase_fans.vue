<template>
  <Wrap class="">
    <!-- 搜索层 -->
    <Search ref="Search" :query-data="queryData" :xingtu-tag-data="xingtuTagData" tip="统计时间段内按照粉丝增量排序" @getData="getBillBoardData" @reset="resetListData" @clear="clear" />
    <!-- 无限list -->
    <ScrollList ref="List" :data="data" style="margin-top: 20px;" type="increase_fans" @getData="getBillBoardData" />
    <!-- 回到顶部 -->
    <BackToTop />
  </Wrap>
</template>
...
// import ScrollItem from '../_com/scroll_item.vue'
...
export default {
  ...
  methods: {
    ...
    async getBillBoardData(pageNum, contentType, cb) {
      const search = this.$refs.Search

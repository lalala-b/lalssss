<template>
  <div class="C-putting-order-my">
    <!-- 筛选 -->
    <orderSearch :plat-data="platData" :account-data="accountData" :delivery-data="deliveryData" :tab-type="type" :business-status="activeName" @getTableData="getSearchData" @updateSearchData="updateSearchData" />
    ...
    getTableData(params, hasOrderLoading = true) {
      ...
      this.getWaitApprovalOrder(params).then(data => {
        ...
        this.$emit('changeTabNum', {
          name: 'WaitOrder',
          val: orderConditionCountVO?.reviewerOrderCount
        })
        this.$emit('setTabOnlyLoading', {
          name: 'WaitOrder',
          val: false

  ...
  <div class="C-putting-order-my">
    ...
    <div>
      ...
      <el-radio-group
        ...
        @change="handleChangeFastScreenRadio"
      >
        <el-radio :label="1">
          是
          <IconTip
...
export default {
  components: {
    orderSearch, orderTable, IconTip
  },
  props: {
    type: {
      type: String,
  ...
  methods: {
    ...
    getTableData(params, hasOrderLoading = true) {
      ...
      this.getWaitApprovalOrder({ ...params, orderPaymentFlag: this.orderPaymentFlag }).then(data => {
        const { orderConditionCountVO } = data
        this.$emit('changeTabNum', {
          name: 'PaymentOrder',
          val: orderConditionCountVO?.waitOrderPayCount
        })
        this.$emit('setTabOnlyLoading', {
          name: 'PaymentOrder',
          val: false
    ...
    handleChangeFastScreenRadio(e) {
      this.pageNum = 1
      this.updateSearchData({ orderPaymentFlag: e })
      this.$nextTick(() => {
        this.getTableData({})
      })
    }

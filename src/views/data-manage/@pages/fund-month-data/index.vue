<template>
  <div>
    ...
    <div v-if="$permission('pirate_putting_month_export_military')" class="chart-export-item">
...
<script>
import qs from 'qs'
import { $getFundTeamData, $getFundCpmData, $getMonthFlowSoilder, $getMonthFundData, $getDeliveryTop } from '@/api/data'
  ...
  methods: {
    handleCommand(command) {
      ...
      switch (command) {
          // 团队军功
          case 'scoreRes':
            window.open(`/api/pirate/data/trend/month/exportFundTeamScoreMonth?${query}`)
            break
          // 视频类型流量军功
          case 'typeFlowSoilder':
            window.open(`/api/pirate/data/trend/month/exportSoldierGroupByJunTypeMonth?${query}`)
            break
          // 团队流量
          case 'flowRes':
            window.open(`/api/pirate/data/trend/month/exportFundTeamFlowMonth?${query}`)
            break
            // 团队流量排行
          case 'fFlow':
    ...
    getPageData() {
      ...
      $getMonthFundData(this.pageTime).then(re => {
        if (re.code === 1) {
    ...
    getTeamData() {
      ...
      $getFundTeamData(this.pageTime).then(res => {
        res = res.data
    ...
    getCpmData() {
      ...
      $getFundCpmData(this.pageTime).then(res => {
    ...
    getRankData() {
      ...
      $getMonthFlowSoilder(this.pageTime).then(re => {
        if (re.code === 1) {
          re = re.data
          // 需要转换数据展示的图表
          const needFormatDataCharts = ['scoreRes', 'typeFlowSoilder']
    ...
    getTopData() {
      ...
      $getDeliveryTop(this.pageTime).then(re => {
        if (re.code === 1) {
          re = re.data
          const keyValueMap = {
            titleFlow: {

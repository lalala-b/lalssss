<template>
  <div>
    ...
    <VTable ref="table" :source="'pending'" :search-data="searchData" :show-receive-batch-btn="showReceiveBatchBtn" />
  </div>
</template>
<script>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const onSubmit = (data) => {
      ...
      // 当审批状态为待接收的时候，显示批量接收按钮
      if (params.transferOrderStatus === 1) {
        states.showReceiveBatchBtn = true
      } else {
        states.showReceiveBatchBtn = false
      }
      handleGetList()
    }

<template>
  <div>
    ...
    <VTable ref="table" :source="'allAgent'" :search-data="searchData" />
  </div>
</template>
<script>
...
import { $getWaitInfo, $getInitiatorList } from '@/api/operating-manage'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const getInitiatorList = () => {
      $getInitiatorList()
        .then((res) => {
          if (res.code === 1) {
            normalFormItemMap.promoterId.data = normalFormItemMap.curHandlerId.data = res.data
          }
        })
        .catch((err) => {
          console.error(err)

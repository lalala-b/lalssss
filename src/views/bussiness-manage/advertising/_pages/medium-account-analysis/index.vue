<template>
  <div>
    <wrap class="search-view">
      ...
      >
        <template v-slot:normal>
          <!-- <el-form-item>
            <el-select v-model="target.model.accountIds" :multiple-limit="5" multiple clearable filterable placeholder="账号">
              <el-option
      ...
      <div class="flex flex-justify-between">
        <div class="flex flex-align-center m-b-16">
          ...
          <IconTip
            class="icon-tip"
            type="popover"
            content="数据来源于最近12个月的媒介订单，金额为媒介订单的刊例价。需要查看全部数据，可使用导出功能。"
            icon-class="iconwenhao"
          />
    ...
    <AccountAnalysisDialog
      ...
      :api="'/api/account/analysis/medium/export'"
...
import { $getMedAccountList, $getMedCondition, $getMedDataList } from '@/api/bussiness-manage'
import qs from 'qs'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getMedAccountList = (platId) => {
      $getMedAccountList({ platId }).then((res) => {
    ...
    const handleExportData = () => {
      const params = formatSearchData(searchData.value)
      window.open(
        `/api/account/analysis/medium/export?${qs.stringify({
          ...params
        })}`
      )
    }

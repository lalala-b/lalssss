...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const handleSelect = (selection, row) => {
      if (row.isChosend) {
        row.isChosend = false
        selectedVideoList.value.splice(selectedVideoList.value.findIndex(item => item.flowId === row.flowId), 1)
      } else {
        if (props.supportMultiVideoFlag === 1) {
          selectedVideoList.value.push(row)
        } else {
          row.isChosend = true
          selectedVideoList.value = [row]
          processCheckedRow()
        }
      }
    }
    ...
    const processCheckedRow = () => {
      ...
      videoTableData.value.forEach(item => {
        ...
        if (isChosend >= 0) {
          ...
          // 老
          $refs.videoTable.toggleRowSelection(item, false)
        }

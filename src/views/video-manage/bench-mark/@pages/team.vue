  ...
  <div class="B-teamVideo">
    <Wrap class="search-view">
      ...
      >
        <template v-slot:normal="target">
          <el-form-item>
            <el-radio-group v-model="nowType" @change="onTypeChange">
              <el-radio-button
          ...
          <platformSelect v-model="target.model.platId" :inline="true" :query="paramsConfig" @platChange="onPlatChange" />
          <accountSelect :inline="true" :list="accountList" :form="target.model" :params="paramsConfig" />
        </template>
      </v-search>
    </Wrap>
    <Wrap v-loading="loading" class="table-view m-t-16">
      ...
      >
        ...
        <el-table-column label="发布时间" prop="addTime" sortable="custom" width="102" />
...
import { $getBenchVideo, $condition, $getTeamCondition } from '@/api/video-manage'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      methodId: 1,
      determined: '-add_time',
      paramsConfig: { methodId: 2, accountType: 2 },
      accountList: [],
      searchData: {
        platId: '',
        f_id: '',
  ...
  methods: {
    async getCondition() {
      ...
      if (res.code === 1) {
        this.normalFormItemMap.note.data = res.data.map(t => {
          return {
            text: t,
            val: t
          }

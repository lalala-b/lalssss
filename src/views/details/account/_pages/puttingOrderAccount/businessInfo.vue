<template>
  <Wrap>
    <ul v-loading="loading" class="list flex ">
      <div class="flex flex-wrap">
        ...
        <template>
          ...
          <li>邮箱：{{ data.email }}</li>
          <li>座机：<template v-if="data.fixedPhoneCode || data.fixedPhoneNumber">{{ data.fixedPhoneCode }}-{{ data.fixedPhoneNumber }}</template></li>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(() => props.data.accountId, (val) => {
      if (val) {
        getDeliveryBusinessInfo()
      }
    })
    const getDeliveryBusinessInfo = async() => {
      const res = await $getDeliveryBusinessInfo({ accountId: props.data.accountId })
      states.loading = false
      if (res.code === 1) {
        states.meetUserTableData = res.data.businessInfos

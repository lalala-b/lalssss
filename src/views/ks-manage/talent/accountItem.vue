  ...
  <div class="list-card flex">
    <div class="list-card__info">
      <AccountCard :info="info" :options="{accountUrl: 'authorUrl'}" :show-tags="false">
        ...
        <p v-if="(info.sysAccountTag || info.tagValue || info.accountTag)" class="account-card__tag">
          ...
          <template v-if="info.accountTag">
            <el-tag v-for="item in info.accountTag.split(',')" :key="item" class="m-r-5 m-b-5" size="mini">{{ item }}</el-tag>
          </template>
        </p>
        <template slot="icon">
          <el-tooltip v-if="info.organizationStarFlag" class="m-r-5" effect="dark" content="机构达人" placement="top">
            <svg-icon icon-class="jigoudaren" />
          </el-tooltip>
          <el-tooltip v-if="info.corporateStarFlag" class="m-r-5" effect="dark" content="支持对公" placement="top">
            <svg-icon icon-class="zhichiduigong" />
          </el-tooltip>
        </template>
      </AccountCard>
      <div class="userInfoId m-t-8">ID：<strong>{{ info.uniqueId || info.eid || info.getDataUrl || '--' }}</strong></div>
      <el-popover
    ...
    <ul class="list-card__data flex1">
      <!-- <li>
      ...
      <template v-if="info.settledFlag">
        <li>
          <h3 data-num>{{ info.expecteExposureNum | toThousandsW }}</h3>
          <p>预期曝光量</p>
        </li>
        <li>
          <h3 data-num>{{ info.expecteClickNum | toThousandsW }}</h3>
          <p>预期播放量</p>
        </li>
        <li>
          <h3 data-num>{{ info.expectCPM | toThousandsW }}</h3>
          <p>预期CPM</p>
        </li>
        <li>
          <h3 data-num>{{ info.fansInteractRate ? (info.fansInteractRate * 100).toFixed(2) + '%' : '--' }}</h3>
          <p>粉丝互动率</p>
        </li>
      </template>
      <li v-else class="flex1 li-tip">达人尚未开通短视频任务</li>
      <!-- <li>
      ...
      <li>
        <h3 data-num>{{ info.cpm !== null ? info.cpm : '--' }}</h3>
        <p>CPM</p>
      </li>
      <li>
    ...
    <div v-if="info.settledFlag" class="list-card__price flex flex-align-center">
      <span data-bold><i v-if="info.sixPrice">¥</i>{{ info.sixPrice | toThousandsW }}</span>
    </div>

  ...
  <div class="page-container">
    <Wrap>
      ...
      <div class="head-info flex">
        <AccountCard :info="accountDetail" class="flex1">
          <span class="account-id">ID：{{ accountDetail.uniqueId || accountDetail.eid || accountDetail.getDataUrl || '--' }}</span>
        </AccountCard>
        <div class="head-info__item flex flex1 flex-dir-col flex-wrap flex-justify-between">
          <div>账号统一名称：{{ accountDetail.belongName || "" }}</div>
          <div>绑定的用户：{{ accountDetail.userList ? accountDetail.userList.join('、') : '' }}</div>
        </div>
        <div class="head-info__item flex flex1 flex-dir-col flex-justify-between">
          <div>账号归属：{{ accountBelong }}</div>
          <div>绑定手机号：{{ accountDetail.bindPhone ? accountDetail.bindPhone.join('、') : '' }}</div>
        </div>
        <div class="head-info__item flex flex1 flex-dir-col flex-justify-between">
          <div>运营负责人：{{ accountDetail.operatorName|emptyFill }}</div>
        </div>
        <!-- <p class="head-info-item">平台: {{ accountDetail.platName || "" }}</p>
        <p class="head-info-item">账号: {{ accountDetail.accountName || "" }}</p>
        <p class="head-info-item">账号统一名称: {{ accountDetail.belongName || "" }}</p>
        <p class="head-info-item">绑定手机号:
          <template v-for="(item, index) in accountDetail.bindPhone">
            <el-tag v-if="item" :key="index" class="user-name">{{ item }}</el-tag>
          </template>
        </p>
        <p class="head-info-item">账号归属: {{ accountBelong }}</p>
        <p class="head-info-item">
          绑定的用户:
          <el-tag v-for="(item, index) in accountDetail.userList" :key="index" class="user-name">{{ item }}</el-tag>
         <PullDown :list="accountDetail.userList" />
         </p> -->
      </div>
    </Wrap>
...
import { Statistics, IconTip, VChart, VideoInfo, AccountCard, Title } from 'components'
import { $accountVideoList, $accountVideoChart, $accountFansChart, $getAccountDetail, $getAccountDataNum } from '@/api/account-manage'
...
export default {
  ...
  computed: {
    ...
    accountBelong() {
      if (this.accountDetail.dname === '暂无' && this.accountDetail.fname === '暂无') {
        if (this.accountType == 5) {
          return '乾饱饱'
        }
      }
      return `${this.accountDetail.dname}-${this.accountDetail.fname}`
    },
